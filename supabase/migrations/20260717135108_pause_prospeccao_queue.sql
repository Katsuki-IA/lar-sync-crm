drop function if exists public.crm_process_next_prospeccao_leads(bigint, integer);
...
begin
  ...
  loop
    ...
    begin
      ...
      if coalesce(p_enqueue, false) = false then
        update public.fila_leads queue
        set status = 'pending',
            attempt_count = greatest(coalesce(queue.attempt_count, 0), 3),
            claimed_at = null,
            claimed_by = null,
            last_error = 'Prospecção pausada até aprovação do fluxo'
        where queue.id_lead = v_created_lead_id::text
          and queue.id_empresa = item.id_empresa
          and queue.crm_provider = 'Hub'
          and coalesce(queue.verificado, 0) = 0
      ...
      insert into public.crm_lead_activities (
        ...
        'site_form',
        case
          when coalesce(p_enqueue, false) = true
            then 'Lead importado para campanha de prospecção.'
          else 'Lead importado para campanha de prospecção. Atendimento pausado até aprovação do fluxo.'
        end,
        jsonb_build_object(
          'source', 'prospeccao_csv',
          'staging_id', item.id,
          ...
          'origem_aba', item.origem_aba,
          'id_empreendimento', item.id_empreendimento,
          'queue_enabled', coalesce(p_enqueue, false)
        )
      );
...
revoke all on function public.crm_process_next_prospeccao_leads(bigint, integer, boolean)
from public, anon, authenticated;

grant execute on function public.crm_process_next_prospeccao_leads(bigint, integer, boolean)
to service_role;
...
set status = 'pending',
    ...
    last_error = 'Prospecção pausada até aprovação do fluxo'
from public.crm_leads lead
where lead.id::text = queue.id_lead
  and lead.id_empresa = queue.id_empresa
  and lead.origem = 'PR'
  and queue.crm_provider = 'Hub'
  and coalesce(queue.verificado, 0) = 0
  and queue.status in ('pending', 'processing');
