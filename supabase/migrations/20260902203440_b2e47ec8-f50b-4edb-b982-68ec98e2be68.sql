...
with bounds as (
  ...
  select l.id_empreendimento as emp_id,
         ...
         count(*) filter (
           where l.created_at >= b.ini and l.created_at < b.fim
             and exists (
               select 1 from public.crm_conversation_classifications c
               where c.lead_id = l.lead_id and c.cliente_respondeu
             )
         ) as respondidos,
...
from (
  select emp.id_empresa as empresa_id,
         ...
         coalesce(agg.respondidos, 0) as leads_respondidos_mes,
         agg.ultima_entrada,
         coalesce((
