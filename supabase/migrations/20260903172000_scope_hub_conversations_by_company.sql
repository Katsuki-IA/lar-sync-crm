...
declare
  ...
  v_scoped_session_candidates text[] := array[]::text[];
  v_legacy_session_candidates text[] := array[]::text[];
  v_raw_phone text;
  v_digits text;
begin
  select
    ...
    l.numero,
    l.id_crm,
    l.wa_identity_id as lead_wa_identity_id,
    l.conversation_key as lead_conversation_key,
    l.legacy_conversation_key as lead_legacy_conversation_key,
    c.telefone as crm_telefone,
    c.wa_identity_id as crm_wa_identity_id,
    c.conversation_key as crm_conversation_key,
    c.legacy_conversation_key as crm_legacy_conversation_key
  into v_lead
  from public.lead as l
  join public.crm_leads as c
  ...
  select array_agg(distinct candidate)
  into v_scoped_session_candidates
  from (
  ...
  select array_agg(distinct phone || v_company_id)
  into v_legacy_session_candidates
  from unnest(v_phone_candidates) as phone
  where phone <> '';
  ...
  where
    (
      ...
      and (
        regexp_replace(coalesce(chat.numero, ''), '\D', '', 'g') = any(v_scoped_session_candidates)
        or (v_lead.lead_wa_identity_id is not null and chat.wa_identity_id = v_lead.lead_wa_identity_id)
        or (v_lead.crm_wa_identity_id is not null and chat.wa_identity_id = v_lead.crm_wa_identity_id)
        or (nullif(v_lead.lead_conversation_key, '') is not null and chat.conversation_key = v_lead.lead_conversation_key)
        or (nullif(v_lead.crm_conversation_key, '') is not null and chat.conversation_key = v_lead.crm_conversation_key)
        or (nullif(v_lead.lead_legacy_conversation_key, '') is not null and chat.legacy_conversation_key = v_lead.lead_legacy_conversation_key)
        or (nullif(v_lead.crm_legacy_conversation_key, '') is not null and chat.legacy_conversation_key = v_lead.crm_legacy_conversation_key)
      )
    )
    or
    (
      chat.id_empresa is null
      and regexp_replace(coalesce(chat.numero, ''), '\D', '', 'g') = any(v_legacy_session_candidates)
    )
  order by
