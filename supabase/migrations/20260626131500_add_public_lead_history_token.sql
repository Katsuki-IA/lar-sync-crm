-- Token publico nao sequencial para links individuais de historico de lead.

alter table public.crm_leads
add column if not exists historico_token uuid;

update public.crm_leads
set historico_token = gen_random_uuid()
where historico_token is null;

alter table public.crm_leads
alter column historico_token set default gen_random_uuid();

alter table public.crm_leads
alter column historico_token set not null;

create unique index if not exists crm_leads_historico_token_key
on public.crm_leads (historico_token);
...
begin
  ...
  where ed.codigo_hub = btrim(p_codigo)
    and (
      l.historico_token::text = btrim(p_lead_ref)
      or (btrim(p_lead_ref) ~ '^[0-9]+$' and l.id = btrim(p_lead_ref)::bigint)
    )
  limit 1;
...
revoke all on function public.crm_public_lead_history(text, text) from public;
